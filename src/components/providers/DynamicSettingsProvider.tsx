...
const DynamicSettingsProvider = ({ children }: DynamicSettingsProviderProps) => {
  ...
  useEffect(() => {
    ...
    const applyAll = () => {
      const fullTitle = `${pageTitle ? `${pageTitle} | ` : ""}${app_name || "ChatWave"} | All-in-One WhatsApp Marketing & Automation Platform with CRM, Campaigns, Live Chat, Lead Generation, Business API SaaS Platform`;
